...
try:
    ...
    from src.config import DatabaseConfig, EmbeddingConfig, ChunkingConfig
    from src.logger import get_vectorstore_logger
except ModuleNotFoundError:
    from embedding import EmbeddingPipeline
    from config import DatabaseConfig, EmbeddingConfig, ChunkingConfig
...
class PgVectorStore:
    def __init__(self,
        ...
        self.embedding_model = embedding_model or EmbeddingConfig.MODEL
        self.chunk_size = chunk_size or ChunkingConfig.SIZE
        self.chunk_overlap = chunk_overlap or ChunkingConfig.OVERLAP
        ...
        if use_reranker:
            try:
                self.reranker = CrossEncoder(EmbeddingConfig.RERANKER_MODEL)
                logger.info(f"Loaded reranker: {EmbeddingConfig.RERANKER_MODEL}")
    ...
    def _init_database(self):
        ...
        try:
            ...
            cur.execute("UPDATE documents SET company_id = 1 WHERE company_id IS NULL;")
            cur.execute("ALTER TABLE documents ALTER COLUMN company_id SET NOT NULL;")
            ...
            cur.execute("UPDATE chunks SET company_id = 1 WHERE company_id IS NULL;")
            cur.execute("ALTER TABLE chunks ALTER COLUMN company_id SET NOT NULL;")
            ...
            cur.execute("UPDATE embeddings SET company_id = 1 WHERE company_id IS NULL;")
            cur.execute("ALTER TABLE embeddings ALTER COLUMN company_id SET NOT NULL;")
        ...
        try:
            ...
            for idx, doc in enumerate(documents):
                ...
                if company_id is None:
                    logger.warning(f"Document '{name}' has no company_id!")
                    continue
                ...
                if self.debug:
                    logger.debug(f"Mapped document '{source}' (company_id={company_id}) to doc_id={doc_id}")
            ...
            for i, chunk in enumerate(chunks):
                ...
                if document_id is None:
                    unmapped_count += 1
                    if self.debug:
                        logger.debug(f"Chunk {i} couldn't be mapped to document. Ref: '{chunk_doc_ref}'")
                    continue
            ...
            if unmapped_count > 0:
                logger.warning(f"{unmapped_count} chunks could not be mapped to documents!")

            # Insert chunks and get IDs
            chunk_ids = []
            logger.info(f"Inserting {len(chunk_data)} chunks...")
            ...
            try:
                ...
                logger.info(f"Inserted {len(chunk_ids)} chunks via batch insert")
                ...
                logger.info(f"Inserted {len(chunk_ids)} chunks via individual inserts")
            ...
            logger.info(f"Inserting {len(embedding_data)} embeddings...")
            execute_values(
                cur,
            ...
            conn.commit()
            logger.info(f"Successfully inserted {len(chunks)} chunks and {len(chunk_ids)} embeddings")
    ...
    def query(self, query_text: str, company_id: int, top_k: int = 15, initial_k: int = 50,
        ...
        if company_id is None:
            logger.error("company_id is required for scoped queries")
            return []
    ...
    def search_bm25(self, query_text: str, company_id: int, top_k: int = 15) -> List[Dict]:
        ...
        try:
            ...
            if not rows:
                if self.debug:
                    logger.debug(f"No chunks found for company_id={company_id}")
                return []
    ...
    def clear(self, _retry_count: int = 0):
        """Clear all data from vector store with proper timeout handling and retry limit"""
        MAX_RETRIES = 3
        conn = self._get_connection()
        cur = conn.cursor()
        try:
            ...
            logger.info("Clearing embeddings...")
            ...
            conn.commit()
            logger.info("Successfully cleared all data from vector store")

        except psycopg2.errors.QueryCanceled:
            conn.rollback()
            if _retry_count >= MAX_RETRIES:
                logger.error(f"Max retries ({MAX_RETRIES}) reached for clear operation")
                raise RuntimeError(f"Failed to clear vector store after {MAX_RETRIES} retries")
            logger.warning(f"Query timeout, retrying ({_retry_count + 1}/{MAX_RETRIES})...")
            time.sleep(2 * (_retry_count + 1))  # Exponential backoff
            self.clear(_retry_count=_retry_count + 1)
