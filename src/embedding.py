...
try:
    ...
    from config import EmbeddingConfig, ChunkingConfig
    from logger import get_logger

# Initialize logger
logger = get_logger('knoccs.embedding')
    ...
    def chunk_documents(self, documents: List[Any]) -> List[Any]:
        ...
        for doc_idx, doc in enumerate(documents):
            ...
            if company_id is None:
                logger.warning(f"Document '{doc_name}' has no company_id assigned!")
                continue
        ...
        logger.info(f"Created {len(all_chunks)} cleaned chunks from {len(documents)} documents")
        ...
        if chunks_without_company:
            logger.warning(f"{len(chunks_without_company)} chunks missing company_id!")
