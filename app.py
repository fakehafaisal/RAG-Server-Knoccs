"""
CLI interface for Knoccs RAG Knowledge Base.
Use this for local testing and interactive querying.
"""

from src.data_loader import load_all_documents
from src.vectorstore import PgVectorStore
from src.search import RAGSearch
from src.config import LLMConfig, ChunkingConfig, SearchConfig, ServerConfig
from src.logger import get_logger

# Initialize logger
logger = get_logger('knoccs.cli')
...
def initialize_rag(debug=False):
    ...
    if store is None:
        store = PgVectorStore(
            chunk_size=ChunkingConfig.SIZE,
            chunk_overlap=ChunkingConfig.OVERLAP,
            use_reranker=True,
            debug=debug
        )
...
def query_knowledge_base(query: str, top_k: int = None, debug=False):
